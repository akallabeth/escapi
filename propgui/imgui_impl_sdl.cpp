...
// ImGui_ImplXXXX_NewFrame(), ImGui::Render() and ImGui_ImplXXXX_Shutdown(). See main.cpp for an
// example of using this. https://github.com/ocornut/imgui
...
void ImGui_ImplSdl_RenderDrawLists(ImDrawData* draw_data)
{
	...
	// A probable faster way to render would be to collate all vertices from all cmd_lists into a
	// single vertex buffer. Setup render state: alpha-blending enabled, no face culling, no depth
...
bool ImGui_ImplSdl_Init(SDL_Window* window)
{
	...
	io.RenderDrawListsFn =
	    ImGui_ImplSdl_RenderDrawLists; // Alternatively you can set this to NULL and call
...
void ImGui_ImplSdl_NewFrame(SDL_Window* window)
{
	...
	if (SDL_GetWindowFlags(window) & SDL_WINDOW_MOUSE_FOCUS)
		io.MousePos = ImVec2((float)mx, (float)my); // Mouse position, in pixels (set to -1,-1 if no
