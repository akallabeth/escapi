...
static ImVec4 GNullClipRect(-8192.0f, -8192.0f, +8192.0f,
...
void ImDrawList::AddCallback(ImDrawCallback callback, void* callback_data)
{
	...
	// Force a new command after us (we function this way so that the most common calls AddLine,
	// AddRect, etc. always have a command to add to without doing any check).
	AddDrawCmd();
}
...
void ImDrawList::UpdateClipRect()
{
	...
	else
	{
		...
		if (CmdBuffer.Size >= 2 &&
		    ImLengthSqr(CmdBuffer.Data[CmdBuffer.Size - 2].ClipRect - current_clip_rect) < 0.00001f)
			CmdBuffer.pop_back();
		else
...
void ImDrawList::UpdateTextureID()
{
	...
	if (!current_cmd || (current_cmd->ElemCount != 0 && current_cmd->TextureId != texture_id) ||
                         ...
                         const ImVec4* cpu_fine_clip_rect)
{
	...
	font->RenderText(font_size, pos, col, clip_rect, text_begin, text_end, this, wrap_width,
...
// This is one of the few function breaking the encapsulation of ImDrawLst, but it is just so
// useful.
                                                    ...
                                                    const ImWchar* glyph_ranges)
{
	...
	return AddFontFromMemoryTTF(buf_decompressed_data, (int)buf_decompressed_size, size_pixels,
	                            font_cfg_template, glyph_ranges);
}
...
bool ImFontAtlas::Build()
{
	...
	TexWidth =
	    (TexDesiredWidth > 0)
	        ...
	        : (total_glyph_count > 2000)
	              ? 2048
	              : (total_glyph_count > 1000)
	                    ? 1024
	                    : 512; // Width doesn't actually matters much but some API/GPU have texture
	...
	for (int input_i = 0; input_i < ConfigData.Size; input_i++)
	{
		...
		float off_y =
		    (cfg.MergeMode && cfg.MergeGlyphCenterV) ? (ascent - dst_font->Ascent) * 0.5f : 0.0f;
                                          ...
                                          float wrap_width) const
{
	...
	while (s < text_end)
	{
		...
		const float char_width =
		    ((int)c < IndexXAdvance.Size) ? IndexXAdvance[(int)c] * scale : FallbackXAdvance;
		if (ImCharIsSpace(c))
		{
			if (inside_word)
			{
...
void ImFont::RenderText(float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect,
                        ...
                        float wrap_width, bool cpu_fine_clip) const
{
	...
	while (s < text_end)
	{
		...
		if (const Glyph* glyph = FindGlyph((unsigned short)c))
		{
			...
			if (c != ' ' && c != '\t')
			{
				// We don't do a second finer clipping test on the Y axis (TODO: do some measurement
				// see if it is worth it, probably not)
				...
				if (x1 <= clip_rect.z && x2 >= clip_rect.x)
				{
					...
					// NB: we are not calling PrimRectUV() here because non-inlined causes too much
					// overhead in a debug build. inlined:
					{
