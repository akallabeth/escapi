...
restriction.
 ...
 - click upper right corner to close a window, available when 'bool* p_opened' is passed to
ImGui::Begin()
...
interface (e.g. if you want a more compact UI or a different color scheme).
 ...
 - getting started:
   - initialisation: call ImGui::GetIO() to retrieve the ImGuiIO structure and fill the 'Settings'
data.
   - every frame:
      1/ in your mainloop or right after you got your keyboard/mouse info, call ImGui::GetIO() and
fill the 'Input' data, then call ImGui::NewFrame(). 2/ use any ImGui function you want between
NewFrame() and Render() 3/ ImGui::Render() to render all the accumulated command-lists. it will call
your RenderDrawListFn handler that you set in the IO structure.
   - all rendering information are stored into command-lists until ImGui::Render() is called.
   - ImGui never touches or know about your GPU state. the only function that knows about GPU is the
RenderDrawListFn handler that you must provide.
   - effectively it means you can create widgets at any time in your code, regardless of "update" vs
...
call GetDrawData() after Render() to access the render data.
        ...
        while (true)
        {
            ...
            // 3) most of your application code here - you can use any of ImGui::* functions at any
point in the frame ImGui::Begin("My window"); ImGui::Text("Hello, world."); ImGui::End();
            GameUpdate();
            GameRender();
   ...
   - after calling ImGui::NewFrame() you can read back flags from the IO structure to tell how ImGui
intends to use your inputs. When 'io.WantCaptureMouse' or 'io.WantCaptureKeyboard' flags are set you
may want to discard/hide the inputs from the rest of your application. When
'io.WantInputsCharacters' is set to may want to notify your OS to popup an on-screen keyboard, if
available.
...
needs to manage the open/closed state of popups. Call OpenPopup() to actually set the "opened" state
...
GetItemRectMin()/GetItemRectMax()/IsMouseHoveringRect(). Kept inline redirection function (will
obsolete).
...
fixes
 ...
 A: Overwrite the following files:
    ...
    Check the "API BREAKING CHANGES" sections for a list of occasional API breaking changes. If you
have a problem with a function, search for its name in the code, there will likely be a comment
about it. Please report any issue to the GitHub page!
...
often needs to remember what is the "active" widget). to do so they need an unique ID. unique ID are
...
actual framebuffer dimension.
 ...
 Q. I integrated ImGui in my engine and some elements are disappearing when I move windows around..
    Most likely you are mishandling the clipping rectangles in your render function. Rectangles
provided by ImGui are defined as (x1,y1,x2,y2) and NOT as (x1,y1,width,height).
...
io.Fonts->GetGlyphRangesJapanese());
 ...
 A: When loading a font, pass custom Unicode ranges to specify the glyphs to load. ImGui will
support UTF-8 encoding across the board. Character input depends on you passing the right character
code to io.AddInputCharacter(). The example applications do that.
...
ImGui!
 ...
 The list below consist mostly of notes of things to do before they are requested/discussed by users
(at that point it usually happens on the github)

 - window: maximum window size settings (per-axis). for large popups in particular user may not want
the popup to fill all space.
...
label-newline-tab-widget etc.
 ...
 - main: considering adding EndFrame()/Init(). some constructs are awkward in the implementation
...
-! menus/popups: clarify usage of popups id, how MenuItem/Selectable closing parent popups affects
the ID, etc. this is quite fishy needs improvement! (#331)
...
real-estate.
 ...
 - text edit: clean up the mess caused by converting UTF-8 <> wchar. the code is rather inefficient
right now.
 - text edit: centered text for slider as input text so it matches typical positioning.
 - text edit: flag to disable live update of the user buffer.
 - text edit: field resize behavior - field could stretch when being edited? hover tooltip shows
more text?
 - tree node / optimization: avoid formatting when clipped.
 - tree node: tree-node/header right-most side doesn't take account of horizontal scrolling.
 - tree node: add treenode/treepush int variants? because (void*) cast from int warns on some
platforms/settings
 - textwrapped: figure out better way to use TextWrapped() in an always auto-resize context
(tooltip, etc.) (git issue #249)
 - settings: write more decent code to allow saving/loading new fields
 - settings: api for per-tool simple persistent data (bool,int,float,columns sizes,etc.) in .ini
file
 - style: store rounded corners in texture to use 1 quad per corner (filled and wireframe). so
rounding have minor cost.
...
active/hovered by offset in HSV space?
...
#pragma clang diagnostic ignored \
    "-Wmissing-noreturn" // warning : function xx could be declared with attribute 'noreturn'
                         // warning    // GetDefaultFontData() asserts which some implementation
                         // makes it never return.
#pragma clang diagnostic \
    ignored "-Wdeprecated-declarations" // warning : 'xx' is deprecated: The POSIX name for this
...
static ImVec2 FindBestPopupWindowPos(const ImVec2& base_pos, const ImVec2& size,
                                     ImGuiWindowFlags flags, int* last_dir, const ImRect& r_inner);
...
static void DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf,
...
// Statically allocated default font atlas. This is merely a maneuver to keep ImFontAtlas definition
// at the bottom of the .h file (otherwise it'd be inside ImGuiIO) Also we wouldn't be able to new()
// one at this point, before users may define IO.MemAllocFn.
...
ImGuiStyle::ImGuiStyle()
{
	...
	ChildWindowRounding =
	    0.0f; // Radius of child window corners rounding. Set to 0.0f to have rectangular windows
	...
	WindowFillAlphaDefault =
	    0.70f;             // Default alpha of window background, if not specified in ImGui::Begin()
	...
	DisplayWindowPadding =
	    ImVec2(22, 22); // Window positions are clamped to be visible within the display area by at
	                    // least this amount. Only covers regular windows.
	DisplaySafeAreaPadding =
	    ImVec2(4, 4); // If you cannot see the edge of your screen (e.g. on a TV) increase the safe
...
void* ImLoadFileToMemory(const char* filename, const char* file_open_mode, int* out_file_size,
                         int padding_bytes)
{
...
void ImGui::ItemSize(const ImVec2& size, float text_offset_y)
{
	...
	window->DC.CursorPos =
	    ImVec2((float)(int)(window->Pos.x + window->DC.ColumnsStartX + window->DC.ColumnsOffsetX),
...
bool ImGui::ItemAdd(const ImRect& bb, const ImGuiID* id)
{
	...
	{
		// Matching the behavior of IsHovered() but ignore if ActiveId==window->MoveID (we clicked
		// on the window background) So that clicking on items with no active id such as Text()
		// still returns true with IsItemHovered()
		window->DC.LastItemHoveredRect = true;
		window->DC.LastItemHoveredAndUsable = false;
		if (g.HoveredRootWindow == window->RootWindow)
			if (g.ActiveId == 0 || (id && g.ActiveId == *id) || g.ActiveIdAllowHoveringOthers ||
...
bool ImGui::IsHovered(const ImRect& bb, ImGuiID id, bool flatten_childs)
{
	...
	if (g.HoveredId == 0 || g.HoveredId == id || g.HoveredIdAllowHoveringOthers)
	{
		...
		if (g.HoveredWindow == window ||
			...
			if ((g.ActiveId == 0 || g.ActiveId == id || g.ActiveIdAllowHoveringOthers) &&
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, bool is_active, bool tab_stop)
{
	...
	if (tab_stop && window->FocusIdxAllRequestNext == IM_INT_MAX &&
	    window->FocusIdxTabRequestNext == IM_INT_MAX && is_active && IsKeyPressedMap(ImGuiKey_Tab))
	{
...
void ImGui::NewFrame()
{
	...
	IM_ASSERT(
	    g.IO.Fonts->Fonts[0]->IsLoaded()); // Font Atlas not created. Did you call
	                                       // io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?
	...
	if ((g.IO.MousePos.x < 0 && g.IO.MousePos.y < 0) ||
	    (g.IO.MousePosPrev.x < 0 &&
	     g.IO.MousePosPrev.y < 0)) // if mouse just appeared or disappeared (negative coordinate) we
	...
	// application and won't request capture.
	...
	bool mouse_owned_by_application =
	    mouse_earliest_button_down != -1 && !g.IO.MouseDownOwned[mouse_earliest_button_down];
	g.IO.WantCaptureMouse = (!mouse_owned_by_application && g.HoveredWindow != NULL) ||
	                        (!mouse_owned_by_application && mouse_any_down) || (g.ActiveId != 0) ||
	                        (!g.OpenedPopupStack.empty()) || (g.CaptureMouseNextFrame);
...
static void AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list, ImDrawList* draw_list)
{
	if (!draw_list->CmdBuffer.empty() && !draw_list->VtxBuffer.empty())
	{
		...
		// Check that draw_list doesn't use more vertices than indexable (default ImDrawIdx = 2
		// bytes = 64K vertices) If this assert triggers because you are drawing lots of stuff
		// manually, A) workaround by calling BeginChild()/EndChild() to put your draw commands in
		// multiple draw lists, B) #define ImDrawIdx to a 'unsigned int' in imconfig.h and render
		// accordingly.
		const unsigned long long int max_vtx_idx = (unsigned long long int)1L
		                                           << (sizeof(ImDrawIdx) * 8);
                       ...
                       bool hide_text_after_hash)
{
	...
	{
		// Render
		window->DrawList->AddText(g.Font, g.FontSize, pos, window->Color(ImGuiCol_Text), text,
		                          text_display_end);
...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text,
                              ...
                              ImGuiAlign align, const ImVec2* clip_min, const ImVec2* clip_max)
{
	// Hide anything after a '##' string
	...
	const ImVec2 text_size = text_size_if_known
	                             ? *text_size_if_known
	                             : ImGui::CalcTextSize(text, text_display_end, false, 0.0f);
...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding)
{
	...
	{
		window->DrawList->AddRect(p_min + ImVec2(1, 1), p_max, window->Color(ImGuiCol_BorderShadow),
		                          rounding);
		window->DrawList->AddRect(p_min, p_max - ImVec2(1, 1), window->Color(ImGuiCol_Border),
		                          rounding);
	}
}
...
// NB: 'items_count' is only used to clamp the result, if you don't know your count you can use
// INT_MAX If you are displaying thousands of items and you have a random access to the list, you
// can perform clipping yourself to save on CPU.
// {
//    float item_height = ImGui::GetTextLineHeightWithSpacing();
//    int display_start, display_end;
//    ImGui::CalcListClipping(count, item_height, &display_start, &display_end);            //
//    calculate how many to clip/display ImGui::SetCursorPosY(ImGui::GetCursorPosY() +
//    (display_start) * item_height);         // advance cursor for (int i = display_start; i <
//    display_end; i++)                                     // display only visible items
//        // TODO: display visible item
//    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + (count - display_end) * item_height);   //
//    advance cursor
// }
...
static ImGuiWindow* FindHoveredWindow(ImVec2 pos, bool excluding_childs)
{
	...
	for (int i = g.Windows.Size - 1; i >= 0; i--)
	{
		...
		ImRect bb(window->ClippedWindowRect.Min - g.Style.TouchExtraPadding,
		          window->ClippedWindowRect.Max + g.Style.TouchExtraPadding);
		if (bb.Contains(pos))
			return window;
	}
	return NULL;
...
{
	ImGuiState& g = *GImGui;
	const bool opened = g.OpenedPopupStack.Size > g.CurrentPopupStack.Size &&
	                    g.OpenedPopupStack[g.CurrentPopupStack.Size].PopupID == id;
	return opened;
}

// One open popup per level of the popup hierarchy (NB: when assigning we reset the Window member of
...
void ImGui::OpenPopup(const char* str_id)
{
	...
	ImGuiPopupRef popup_ref =
	    ImGuiPopupRef(id, window, window->GetID("##menus"),
	                  g.IO.MousePos); // Tagged as new ref because constructor sets Window to NULL
	                                  // (we are passing the ParentWindow info here)
	...
	if (g.FocusedWindow)
	{
		for (n = 0; n < g.OpenedPopupStack.Size; n++)
		{
			...
			for (int m = n; m < g.OpenedPopupStack.Size && !has_focus; m++)
				has_focus =
				    (g.OpenedPopupStack[m].Window &&
				     g.OpenedPopupStack[m].Window->RootWindow == g.FocusedWindow->RootWindow);
			if (!has_focus)
				break;
		}
	}
	if (n < g.OpenedPopupStack.Size) // This test is not required but it allows to set a useful
...
void ImGui::CloseCurrentPopup()
{
	...
	if (popup_idx < 0 || popup_idx > g.OpenedPopupStack.Size ||
	    g.CurrentPopupStack[popup_idx].PopupID != g.OpenedPopupStack[popup_idx].PopupID)
		return;
	while (popup_idx > 0 && g.OpenedPopupStack[popup_idx].Window &&
	       (g.OpenedPopupStack[popup_idx].Window->Flags & ImGuiWindowFlags_ChildMenu))
		popup_idx--;
...
static void ClearSetNextWindowData()
{
	...
	g.SetNextWindowPosCond = g.SetNextWindowSizeCond = g.SetNextWindowContentSizeCond =
	    g.SetNextWindowCollapsedCond = g.SetNextWindowFocus = 0;
}
...
static bool BeginPopupEx(const char* str_id, ImGuiWindowFlags extra_flags)
{
	...
	if (flags & ImGuiWindowFlags_ChildMenu)
		...
		ImFormatString(name, 20, "##popup_%08x",
	...
	if (!opened) // opened can be 'false' when the popup is completely clipped (e.g. zero size
	             // display)
		ImGui::EndPopup();
...
bool ImGui::BeginPopupModal(const char* name, bool* p_opened, ImGuiWindowFlags extra_flags)
{
	...
	if (!opened || (p_opened && !*p_opened)) // Opened can be 'false' when the popup is completely
                       ...
                       ImGuiWindowFlags extra_flags)
{
	...
	if (size.x <= 0.0f)
	{
		...
		size.x =
		    ImMax(content_avail.x, 4.0f) - fabsf(size.x); // Arbitrary minimum zero-ish child size
		                                                  // of 4.0f (0.0f causing too much issues)
	}
	if (size.y <= 0.0f)
	{
		if (size.y == 0.0f)
...
void ImGui::EndChild()
{
	...
	else
	{
		...
		if (window->Flags &
		    ImGuiWindowFlags_ChildWindowAutoFitX) // Arbitrary minimum zeroish child size of 4.0f
                                     ...
                                     ImGuiWindowFlags flags, int* last_dir, const ImRect& r_inner)
{
	...
	for (int n = (*last_dir != -1) ? -1 : 0; n < 4;
	     n++) // Right, down, up, left. Favor last used direction.
	{
	...
	*last_dir = -1;
	if (flags &
	    ImGuiWindowFlags_Tooltip) // For tooltip we prefer avoiding the cursor at all cost even if
	                              // it means that part of the tooltip won't be visible.
...
// - Passing 'bool* p_opened' displays a Close button on the upper-right corner of the window, the
...
bool ImGui::Begin(const char* name, bool* p_opened, const ImVec2& size_on_first_use, float bg_alpha,
                  ImGuiWindowFlags flags)
{
	...
	bool window_was_active = (window->LastFrameActive ==
	...
	if (g.SetNextWindowPosCond)
	{
		const ImVec2 backup_cursor_pos =
		    window->DC.CursorPos; // FIXME: not sure of the exact reason of this anymore :( need to
		                          // look into that.
	...
	window->RootNonPopupWindow =
	    g.CurrentWindowStack[root_non_popup_idx]; // This is merely for displaying the TitleBgActive
	                                              // color.
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			size_auto_fit =
			    ImClamp(window->SizeContents + window->WindowPadding, style.WindowMinSize,
			            ImMax(style.WindowMinSize, g.IO.DisplaySize - window->WindowPadding));
		...
		if ((flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup))
		{
			...
			window->Size = window->SizeFull =
			    size_on_first_use; // NB: argument name 'size_on_first_use' misleading here, it's
			                       // really just 'size' as provided by user.
		}
		...
		if (window_pos_center)
		{
			...
			SetWindowPos(ImMax(style.DisplaySafeAreaPadding,
		...
		else if (flags & ImGuiWindowFlags_ChildMenu)
		{
			...
			window->PosFloat = FindBestPopupWindowPos(window->PosFloat, window->Size, flags,
		...
		if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api)
		{
			...
			window->PosFloat = FindBestPopupWindowPos(g.IO.MousePos, window->Size, flags,
			                                          &window->AutoPosLastDirection, rect_to_avoid);
		}

		// User moving window (at the beginning of the frame to avoid input lag or sheering). Only
		// valid for root windows.
		...
		window->FocusIdxAllRequestCurrent =
		    (window->FocusIdxAllRequestNext == IM_INT_MAX || window->FocusIdxAllCounter == -1)
		        ? IM_INT_MAX
		        : (window->FocusIdxAllRequestNext + (window->FocusIdxAllCounter + 1)) %
		              (window->FocusIdxAllCounter + 1);
		window->FocusIdxTabRequestCurrent =
		    (window->FocusIdxTabRequestNext == IM_INT_MAX || window->FocusIdxTabCounter == -1)
		        ? IM_INT_MAX
		...
		if ((flags & ImGuiWindowFlags_Modal) != 0 && window == GetFrontMostModalRootWindow())
		{
			...
			window->DrawList->AddRectFilled(
			    ...
			    window->Color(ImGuiCol_ModalWindowDarkening, g.ModalWindowDarkeningRatio));
		}
		...
		{
			// Draw title bar only
			window->DrawList->AddRectFilled(title_bar_rect.GetTL(), title_bar_rect.GetBR(),
			                                window->Color(ImGuiCol_TitleBgCollapsed),
			                                window_rounding);
			if (flags & ImGuiWindowFlags_ShowBorders)
			{
				window->DrawList->AddRect(title_bar_rect.GetTL() + ImVec2(1, 1),
				                          title_bar_rect.GetBR() + ImVec2(1, 1),
				                          window->Color(ImGuiCol_BorderShadow), window_rounding);
				window->DrawList->AddRect(title_bar_rect.GetTL(), title_bar_rect.GetBR(),
		...
		else
		{
			    ...
			    window->AutoFitFramesY <= 0 && !(flags & ImGuiWindowFlags_NoResize))
			{
				...
				resize_col = window->Color(held ? ImGuiCol_ResizeGripActive
				...
				{
					window->SizeFull =
					    ImMax(window->SizeFull + g.IO.MouseDelta, style.WindowMinSize);
					if (!(flags & ImGuiWindowFlags_NoSavedSettings))
						MarkSettingsDirty();
				}
			...
			// Scrollbars
			window->ScrollbarY = (window->SizeContents.y > window->Size.y + style.ItemSpacing.y) &&
			                     !(flags & ImGuiWindowFlags_NoScrollbar);
			window->ScrollbarX =
			...
			if (bg_alpha > 0.0f)
			{
				if ((flags & ImGuiWindowFlags_ComboBox) != 0)
					window->DrawList->AddRectFilled(window->Pos, window->Pos + window->Size,
					                                window->Color(ImGuiCol_ComboBg, bg_alpha),
					                                window_rounding);
				else if ((flags & ImGuiWindowFlags_Tooltip) != 0)
					window->DrawList->AddRectFilled(window->Pos, window->Pos + window->Size,
					                                window->Color(ImGuiCol_TooltipBg, bg_alpha),
					                                window_rounding);
				else if ((flags & ImGuiWindowFlags_Popup) != 0)
					window->DrawList->AddRectFilled(window->Pos, window->Pos + window->Size,
					                                window->Color(ImGuiCol_WindowBg, bg_alpha),
					                                window_rounding);
				else if ((flags & ImGuiWindowFlags_ChildWindow) != 0)
					window->DrawList->AddRectFilled(window->Pos, window->Pos + window->Size,
					                                window->Color(ImGuiCol_ChildWindowBg, bg_alpha),
					                                window_rounding);
				else
					window->DrawList->AddRectFilled(window->Pos, window->Pos + window->Size,
					                                window->Color(ImGuiCol_WindowBg, bg_alpha),
			...
			if (!(flags & ImGuiWindowFlags_NoTitleBar))
				window->DrawList->AddRectFilled(
				    ...
				    window->Color((g.FocusedWindow && window->RootNonPopupWindow ==
			...
			if (flags & ImGuiWindowFlags_MenuBar)
			{
				...
				window->DrawList->AddRectFilled(
				    menu_bar_rect.GetTL(), menu_bar_rect.GetBR(), window->Color(ImGuiCol_MenuBarBg),
			...
			if (flags & ImGuiWindowFlags_ShowBorders)
			{
				...
				window->DrawList->AddRect(window->Pos, window->Pos + window->Size,
				                          window->Color(ImGuiCol_Border), window_rounding);
				if (!(flags & ImGuiWindowFlags_NoTitleBar))
					window->DrawList->AddLine(title_bar_rect.GetBL(), title_bar_rect.GetBR(),
			...
			if (!(flags & ImGuiWindowFlags_NoResize))
			{
				...
				window->DrawList->PathArcToFast(
				    ImVec2(br.x - window_rounding, br.y - window_rounding), window_rounding, 0, 3);
		...
		window->DC.CursorStartPos =
		    window->Pos + ImVec2(window->DC.ColumnsStartX + window->DC.ColumnsOffsetX,
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			ImVec2 text_max = window->Pos + ImVec2(window->Size.x - style.FramePadding.x,
			                                       style.FramePadding.y * 2 + text_size.y);
			ImVec2 clip_max =
			    ImVec2(window->Pos.x + window->Size.x -
			               (p_opened ? title_bar_rect.GetHeight() - 3 : style.FramePadding.x),
	...
	ImRect clip_rect(title_bar_rect.Min.x + 0.5f + window->WindowPadding.x * 0.5f,
	                 title_bar_rect.Max.y + window->MenuBarHeight() + 0.5f,
	                 window->Pos.x + window->Size.x - window->WindowPadding.x * 0.5f,
	...
	if (flags & ImGuiWindowFlags_ChildWindow)
	{
		...
		if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0 &&
			...
			window->Collapsed |= (window->ClipRect.Min.x >= window->ClipRect.Max.x ||
			                      window->ClipRect.Min.y >= window->ClipRect.Max.y);
...
static void Scrollbar(ImGuiWindow* window, bool horizontal)
{
	...
	ImRect bb = horizontal
	                ? ImRect(window->Pos.x + border_offset, window_rect.Max.y - style.ScrollbarSize,
	                         window_rect.Max.x - other_scrollbar_size_w, window_rect.Max.y)
	                : ImRect(window_rect.Max.x - style.ScrollbarSize, window->Pos.y + border_offset,
	                         window_rect.Max.x, window_rect.Max.y - other_scrollbar_size_w);
	if (!horizontal)
		bb.Min.y += window->TitleBarHeight() + ((window->Flags & ImGuiWindowFlags_MenuBar)
		                                            ? window->MenuBarHeight() - border_offset
		                                            : 0.0f);
	...
	if (horizontal)
		...
		window_rounding_corners =
		    ((window->Flags & ImGuiWindowFlags_NoTitleBar) ? 2 : 0) | (other_scrollbar ? 0 : 4);
	...
	const ImU32 grab_col =
	    window->Color(held ? ImGuiCol_ScrollbarGrabActive
...
static void PushMultiItemsWidths(int components, float w_full)
{
	...
	const float w_item_one = ImMax(
	    1.0f, (float)(int)((w_full - (style.FramePadding.x * 2.0f + style.ItemInnerSpacing.x) *
	                                     (components - 1)) /
	                       (float)components));
	const float w_item_last =
	    ImMax(1.0f, (float)(int)(w_full - (w_item_one + style.FramePadding.x * 2.0f +
...
static void SetWindowPos(ImGuiWindow* window, const ImVec2& pos, ImGuiSetCond cond)
{
	...
	window->DC.CursorPos +=
	    (window->Pos -
	     old_pos); // As we happen to move the window while it is being appended to (which is a bad
	               // idea - will smear) let's at least offset the cursor
	window->DC.CursorMaxPos +=
	    (window->Pos - old_pos); // And more importantly we need to adjust this so size calculation
...
{
	ImGuiWindow* window = GetCurrentWindowRead();
	ImVec2 content_region_size =
	    ImVec2(window->SizeContentsExplicit.x ? window->SizeContentsExplicit.x
	                                          : window->Size.x - window->ScrollbarSizes.x,
	           window->SizeContentsExplicit.y ? window->SizeContentsExplicit.y
...
{
	ImGuiWindow* window = GetCurrentWindowRead();
	return ImVec2(-window->Scroll.x,
	              -window->Scroll.y + window->TitleBarHeight() + window->MenuBarHeight()) +
...
ImVec2 ImGui::GetWindowContentRegionMax()
{
	ImGuiWindow* window = GetCurrentWindowRead();
	ImVec2 content_region_size =
	    ImVec2(window->SizeContentsExplicit.x ? window->SizeContentsExplicit.x : window->Size.x,
	           window->SizeContentsExplicit.y ? window->SizeContentsExplicit.y : window->Size.y);
	ImVec2 m =
	    content_region_size - window->Scroll - window->WindowPadding - window->ScrollbarSizes;
	return m;
}
...
void ImGui::SetScrollY(float scroll_y)
{
	...
	window->ScrollTarget.y =
	    scroll_y +
	    window->TitleBarHeight(); // title bar height canceled out when using ScrollTargetRelY
...
void ImGui::SetScrollFromPosY(float pos_y, float center_y_ratio)
{
	...
	if (center_y_ratio <= 0.0f &&
	    window->ScrollTarget.y <=
	        window->WindowPadding
	            .y) // Minor hack to make "scroll to top" take account of WindowPadding, else it
	                // would scroll to (WindowPadding.y - ItemSpacing.y)
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
	...
	const ImRect value_bb(window->DC.CursorPos,
	                      window->DC.CursorPos + ImVec2(w + style.FramePadding.x * 2,
	                                                    label_size.y + style.FramePadding.y * 2));
	const ImRect total_bb(window->DC.CursorPos,
	                      window->DC.CursorPos +
	                          ImVec2(w + style.FramePadding.x * 2 +
	                                     (label_size.x > 0.0f ? style.ItemInnerSpacing.x : 0.0f),
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held,
                           bool allow_key_modifiers, ImGuiButtonFlags flags)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	...
	if (hovered)
	{
		...
		if (allow_key_modifiers || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
		{
			...
			else if ((flags & ImGuiButtonFlags_Repeat) && g.ActiveId == id &&
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	// Render
	const ImU32 col =
	    window->Color((hovered && held) ? ImGuiCol_ButtonActive
...
static bool CloseWindowButton(bool* p_opened)
{
	...
	const ImRect bb(window->Rect().GetTR() + ImVec2(-2.0f - size, 2.0f),
	                window->Rect().GetTR() + ImVec2(-2.0f, 2.0f + size));

	bool hovered, held;
	bool pressed = ImGui::ButtonBehavior(bb, id, &hovered, &held, true);

	// Render
	const ImU32 col = window->Color((held && hovered) ? ImGuiCol_CloseButtonActive
...
bool ImGui::TreeNodeBehaviorIsOpened(ImGuiID id, ImGuiTreeNodeFlags flags)
{
	...
	// nodes to be logged.
	if (g.LogEnabled && !(flags & ImGuiTreeNodeFlags_NoAutoExpandOnLog) &&
...
bool ImGui::CollapsingHeader(const char* label, const char* str_id, bool display_frame,
                             bool default_open)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
	...
	const ImRect text_bb(bb.Min, bb.Min + ImVec2(collapser_width + style.FramePadding.x * 2 * 0 +
	                                                 (label_size.x > 0.0f ? label_size.x : 0.0f),
	                                             label_size.y));
	ItemSize(ImVec2(text_bb.GetSize().x, bb.GetSize().y),
	         display_frame ? style.FramePadding.y : 0.0f);

	const ImRect interact_bb =
	    display_frame
	        ? bb
	        : ImRect(text_bb.Min, text_bb.Max + ImVec2(style.FramePadding.x * 2, 0.0f)); // FIXME
	bool opened = TreeNodeBehaviorIsOpened(
	    id, (default_open ? ImGuiTreeNodeFlags_DefaultOpen : 0) |
	            (display_frame ? ImGuiTreeNodeFlags_NoAutoExpandOnLog : 0));
	...
	// Render
	const ImU32 col =
	    window->Color((held && hovered) ? ImGuiCol_HeaderActive
	...
	if (display_frame)
	{
		...
		RenderTextClipped(bb.Min + style.FramePadding + ImVec2(collapser_width, 0), bb.Max, label,
		                  NULL, &label_size);
		if (g.LogEnabled)
		{
	...
	else
	{
		...
		RenderCollapseTriangle(bb.Min + ImVec2(style.FramePadding.x, g.FontSize * 0.15f), opened,
		                       0.70f, false);
		if (g.LogEnabled)
...
void ImGui::Bullet()
{
	...
	window->DrawList->AddCircleFilled(
	    bb.Min + ImVec2(style.FramePadding.x + line_height * 0.5f, line_height * 0.5f), bullet_size,
	    window->Color(ImGuiCol_Text));
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
	...
	const ImRect bb(
	    window->DC.CursorPos,
	    window->DC.CursorPos +
	        ImVec2(line_height + (label_size.x > 0.0f ? (style.FramePadding.x * 2) : 0.0f), 0) +
	        label_size); // Empty text doesn't add padding
	ItemSize(bb);
	if (!ItemAdd(bb, NULL))
		return;
	...
	else if (data_type == ImGuiDataType_Float)
	{
		if (decimal_precision < 0)
			ImFormatString(
			    buf, buf_size, "%f",
			    *(float*)
			        data_ptr); // Ideally we'd have a minimum decimal precision of 1 to visually
			                   // denote that it is a float, while hiding non-significant digits?
		else
                                    ...
                                    const char* scalar_format)
{
	...
	if (data_type == ImGuiDataType_Int)
	{
		...
		{
			if (sscanf(buf, "%f", &op_v) == 1 && op_v != 0.0f)
				*v = (int)(ref_v / op_v);
		} // Divide
		else
		{
			if (sscanf(buf, scalar_format, &ref_v) == 1)
				*v = ref_v;
		} // Assign constant
	}
	else if (data_type == ImGuiDataType_Float)
                                           ...
                                           int decimal_precision)
{
	...
	bool value_changed =
	    InputTextEx(label, buf, IM_ARRAYSIZE(buf), aabb.GetSize() - g.Style.FramePadding * 2.0f,
	...
	if (value_changed)
		DataTypeApplyOpFromText(buf, GImGui->InputTextState.InitialText.begin(), data_type,
		                        data_ptr, NULL);
	return value_changed;
}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max,
                           float power, int decimal_precision, bool horizontal)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
	...
	const float slider_sz = horizontal ? (frame_bb.GetWidth() - grab_padding * 2.0f)
	...
	if (decimal_precision > 0)
		...
		grab_sz =
		    ImMin(ImMax(1.0f * (slider_sz / (v_max - v_min + 1.0f)), style.GrabMinSize),
		          slider_sz); // Integer sliders, if possible have the grab size represent 1 unit
	const float slider_usable_sz = slider_sz - grab_sz;
	const float slider_usable_pos_min =
	    (horizontal ? frame_bb.Min.x : frame_bb.Min.y) + grab_padding + grab_sz * 0.5f;
	const float slider_usable_pos_max =
	    (horizontal ? frame_bb.Max.x : frame_bb.Max.y) - grab_padding - grab_sz * 0.5f;
	...
	window->DrawList->AddRectFilled(
	    ...
	    window->Color(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab),
	    style.GrabRounding);
                        ...
                        const char* display_format, float power)
{
	...
	const bool value_changed =
	    SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, true);
                         ...
                         const char* display_format, float power)
{
	...
	// Actual slider behavior + render grab
	bool value_changed =
	    SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, false);
	...
	RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max,
	                  value_buf, value_buf_end, NULL, ImGuiAlign_Center);
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max)
{
	...
	bool value_changed =
	    ImGui::SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, "%.0f deg", 1.0f);
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* display_format)
{
	...
	float v_f = (float)*v;
	bool value_changed =
	    ImGui::SliderFloat(label, &v_f, (float)v_min, (float)v_max, display_format, 1.0f);
	*v = (int)v_f;
	return value_changed;
}
                       ...
                       const char* display_format)
{
	...
	float v_f = (float)*v;
	bool value_changed =
	    ImGui::VSliderFloat(label, size, &v_f, (float)v_min, (float)v_max, display_format, 1.0f);
	*v = (int)v_f;
	return value_changed;
}
                         ...
                         float v_max, int decimal_precision, float power)
{
	...
	// Draw frame
	const ImU32 frame_col = window->Color(
                       ...
                       float v_max, const char* display_format, float power)
{
	...
	for (int i = 0; i < components; i++)
	{
		ImGui::PushID(i);
		value_changed |=
		    ImGui::DragFloat("##v", &v[i], v_speed, v_min, v_max, display_format, power);
		ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
		ImGui::PopID();
		ImGui::PopItemWidth();
	}
                            ...
                            const char* display_format_max, float power)
{
	...
	ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
	value_changed |=
	    ImGui::DragFloat("##max", v_current_max, v_speed,
                    ...
                    const char* display_format)
{
	...
	float v_f = (float)*v;
	bool value_changed =
	    ImGui::DragFloat(label, &v_f, v_speed, (float)v_min, (float)v_max, display_format);
	*v = (int)v_f;
	return value_changed;
}
                          ...
                          const char* display_format_max)
{
	...
	bool value_changed = ImGui::DragInt(
	    "##min", v_current_min, v_speed, (v_min >= v_max) ? IM_INT_MIN : v_min,
	    (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max), display_format);
	ImGui::PopItemWidth();
	ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
	value_changed |=
	    ImGui::DragInt("##max", v_current_max, v_speed,
	                   (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min),
	                   (v_min >= v_max) ? IM_INT_MAX : v_max,
                   ...
                   ImVec2 graph_size)
{
	...
	const ImU32 col_hovered =
	    window->Color((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLinesHovered
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (*v)
	{
		...
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad),
		                                ...
		                                window->Color(ImGuiCol_CheckMark), style.FrameRounding);
	}
...
bool ImGui::RadioButton(const char* label, bool active)
{
	...
	window->DrawList->AddCircleFilled(
	    center, radius,
	    window->Color((held && hovered) ? ImGuiCol_FrameBgActive
	                                    : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg),
	    16);
	if (active)
	{
		const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
		const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
		window->DrawList->AddCircleFilled(center, radius - pad, window->Color(ImGuiCol_CheckMark),
		                                  16);
	}
	...
	if (window->Flags & ImGuiWindowFlags_ShowBorders)
	{
		window->DrawList->AddCircle(center + ImVec2(1, 1), radius,
		                            window->Color(ImGuiCol_BorderShadow), 16);
...
#define STB_TEXTEDIT_IS_SPACE(CH) \
	(ImCharIsSpace((unsigned int)CH) || is_separator((unsigned int)CH))
...
// FIXME: This is rather complex partly because we are doing UTF8 > u16 > UTF8 conversions on the go
// to more easily handle stb_textedit calls. Ideally we should stay in UTF-8 all the time.
                        ...
                        ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	ImVec2 size = CalcItemSize(
	    ...
	    is_multiline ? ImGui::GetTextLineHeight() * 8.0f
	                 : label_size.y); // Arbitrary default of 8 lines high for multi-line
	const ImRect frame_bb(window->DC.CursorPos,
	                      window->DC.CursorPos + size + style.FramePadding * 2.0f);
	const ImRect total_bb(
	    frame_bb.Min,
	    frame_bb.Max +
	        ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
	...
	const bool user_scrolled = is_multiline && g.ActiveId == 0 && edit_state.Id == id &&
	                           g.ActiveIdPreviousFrame == draw_window->GetID("#SCROLLY");
	...
	if (focus_requested || user_clicked || user_scrolled)
	{
		if (g.ActiveId != id)
		{
			...
			// to wchar) From the moment we focused we are ignoring the content of 'buf'
	...
	if (g.ActiveId == id)
	{
		...
		// interacting right now with the widget. Down the line we should have a cleaner concept of
		// focused vs active in the library.
		g.ActiveIdAllowHoveringOthers = !io.MouseDown[0];

		// Edit in progress
		const float mouse_x =
		    (g.IO.MousePos.x - frame_bb.Min.x - style.FramePadding.x) + edit_state.ScrollX;
		const float mouse_y =
		    (is_multiline ? (g.IO.MousePos.y - draw_window->DC.CursorPos.y - style.FramePadding.y)
		                  : (g.FontSize * 0.5f));
		...
		if (g.IO.InputCharacters[0])
		{
			...
			{
				for (int n = 0; n < IM_ARRAYSIZE(g.IO.InputCharacters) && g.IO.InputCharacters[n];
				     n++)
		...
		{
			edit_state.OnKeyPressed(is_ctrl_down ? STB_TEXTEDIT_K_WORDLEFT | k_mask
			                                     : STB_TEXTEDIT_K_LEFT | k_mask);
		}
		else if (IsKeyPressedMap(ImGuiKey_RightArrow))
		{
			edit_state.OnKeyPressed(is_ctrl_down ? STB_TEXTEDIT_K_WORDRIGHT | k_mask
		...
		else
		{
			...
			// Note that as soon as we can focus into the input box, the in-widget value gets
			      ...
			      ImGuiInputTextFlags_CallbackAlways)) != 0)
			{
				...
				if (event_key != ImGuiKey_COUNT ||
				    (flags & ImGuiInputTextFlags_CallbackAlways) != 0)
				{
					ImGuiTextEditCallbackData callback_data;
					...
					if (callback_data.BufDirty)
					{
						edit_state.CurLenW = ImTextStrFromUtf8(
						    text, edit_state.Text.Size, edit_state.TempTextBuffer.Data, NULL);
	...
	const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y,
	                       frame_bb.Min.x + size.x + style.FramePadding.x * 2.0f,
	                       frame_bb.Min.y + size.y + style.FramePadding.y * 2.0f);
	ImVec2 render_pos =
	    is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding;
	ImVec2 text_size(0.f, 0.f);
	if (g.ActiveId == id ||
	    (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY")))
	{
		...
		draw_window->DrawList->AddText(
		    g.Font, g.FontSize, render_pos - render_scroll, draw_window->Color(ImGuiCol_Text), buf,
		    buf + edit_state.CurLenA, 0.0f, is_multiline ? NULL : &clip_rect);
		...
		if (cursor_is_visible)
			draw_window->DrawList->AddLine(cursor_screen_pos + ImVec2(0.0f, -g.FontSize + 0.5f),
			                               cursor_screen_pos + ImVec2(0.0f, -1.5f),
			                               window->Color(ImGuiCol_Text));

		// Notify OS of text input position for advanced IME
		if (is_editable && io.ImeSetInputScreenPosFn &&
		    ImLengthSqr(edit_state.InputCursorScreenPos - cursor_screen_pos) > 0.0001f)
			io.ImeSetInputScreenPosFn(
			    (int)cursor_screen_pos.x - 1,
			    (int)(cursor_screen_pos.y -
			          g.FontSize)); // -1 x offset so that Windows IME can cover our cursor. Bit of
			                        // an extra nicety.
	...
	else
	{
		...
		const char* buf_end = NULL;
		if (is_multiline)
			text_size = ImVec2(size.x, InputTextCalcTextLenAndLineCount(buf, &buf_end) *
			                               g.FontSize); // We don't need width
		draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos,
		                               draw_window->Color(ImGuiCol_Text), buf, buf_end, 0.0f,
...
bool ImGui::InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& size,
                               ...
                               void* user_data)
{
	bool ret = InputTextEx(label, buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline,
	                       callback, user_data);
	return ret;
}
                       ...
                       int decimal_precision, ImGuiInputTextFlags extra_flags)
{
	...
	return InputScalarEx(label, ImGuiDataType_Float, (void*)v, (void*)(step > 0.0f ? &step : NULL),
	                     (void*)(step_fast > 0.0f ? &step_fast : NULL), display_format,
	                     extra_flags);
}
...
bool ImGui::Combo(const char* label, int* current_item, const char** items, int items_count,
                  int height_in_items)
{
	const bool value_changed =
                  ...
                  int height_in_items)
{
	...
	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max,
	            window->Color(hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true,
	...
	if (IsPopupOpen(id))
	{
		...
		ImRect popup_rect(ImVec2(frame_bb.Min.x, frame_bb.Max.y),
		                  ImVec2(frame_bb.Max.x, frame_bb.Max.y + popup_height));
		popup_rect.Max.y = ImMin(
		    popup_rect.Max.y,
		    g.IO.DisplaySize.y - style.DisplaySafeAreaPadding
		                             .y); // Adhoc height limit for Combo. Ideally should be handled
		                                  // in Begin() along with other popups size, we want to
		                                  // have the possibility of moving the popup above as well.
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
	...
	ImVec2 size = CalcItemSize(size_arg, CalcItemWidth() + style.FramePadding.x * 2.0f,
...
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items)
{
	...
	size.x = 0.0f;
	size.y =
	    ImGui::GetTextLineHeightWithSpacing() * height_in_items_f + ImGui::GetStyle().ItemSpacing.y;
...
bool ImGui::ListBox(const char* label, int* current_item, const char** items, int items_count,
                    int height_items)
{
	const bool value_changed =
...
bool ImGui::BeginMainMenuBar()
{
	...
	if (!ImGui::Begin("##MainMenuBar", NULL,
...
bool ImGui::BeginMenuBar()
{
	...
	PushClipRect(ImVec4(rect.Min.x + 0.5f, rect.Min.y - 0.5f, rect.Max.x + 0.5f, rect.Max.y - 1.5f),
	             false);
	window->DC.CursorPos =
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
	...
	bool menuset_opened =
	    !(window->Flags & ImGuiWindowFlags_Popup) &&
	    (g.OpenedPopupStack.Size > g.CurrentPopupStack.Size &&
	     g.OpenedPopupStack[g.CurrentPopupStack.Size].ParentMenuSet == window->GetID("##menus"));
	if (menuset_opened)
		g.FocusedWindow = window;
	...
	if (window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu))
	{
		...
		// using timers so menus feel more reactive.
		bool moving_within_opened_triangle = false;
		if (g.HoveredWindow == window && g.OpenedPopupStack.Size > g.CurrentPopupStack.Size &&
		    g.OpenedPopupStack[g.CurrentPopupStack.Size].ParentWindow == window)
		{
			if (ImGuiWindow* next_window = g.OpenedPopupStack[g.CurrentPopupStack.Size].Window)
			{
				...
				tb.y = ta.y + ImMax((tb.y - extra) - ta.y,
				                    -100.0f); // triangle is maximum 200 high to limit the slope and
				...
				// 0x80008000 : 0x80000080); window->DrawList->PopClipRect(); // Debug
			}
		}

		want_close =
		    (opened && !hovered && g.HoveredWindow == window && g.HoveredIdPreviousFrame != 0 &&
		     g.HoveredIdPreviousFrame != id && !moving_within_opened_triangle);
		want_open = (!opened && hovered && !moving_within_opened_triangle) ||
		            (!opened && hovered && pressed);
	}
	...
	else if (pressed || (hovered && menuset_opened &&
	                     !opened)) // menu-bar: first click to open, then hover to open others
		want_open = true;
	...
	if (opened)
	{
		...
		opened = BeginPopupEx(label, flags); // opened can be 'false' when the popup is completely
		                                     // clipped (e.g. zero size display)
	}
...
bool ImGui::ColorButton(const ImVec4& col, bool small_height, bool outline_border)
{
	...
	if (hovered)
		ImGui::SetTooltip("Color:\n(%.2f,%.2f,%.2f,%.2f)\n#%02X%02X%02X%02X", col.x, col.y, col.z,
		                  col.w, IM_F32_TO_INT8(col.x), IM_F32_TO_INT8(col.y),
		                  IM_F32_TO_INT8(col.z), IM_F32_TO_INT8(col.z));
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool alpha)
{
	...
	int i[4] = { IM_F32_TO_INT8(f[0]), IM_F32_TO_INT8(f[1]), IM_F32_TO_INT8(f[2]),
		         IM_F32_TO_INT8(f[3]) };
	...
	switch (edit_mode)
	{
		...
		case ImGuiColorEditMode_HSV:
		{
			...
			const float w_item_one =
			    ImMax(1.0f, (float)(int)((w_items_all -
			                             ...
			                             (float)components));
			const float w_item_last =
			    ImMax(1.0f, (float)(int)(w_items_all - (w_item_one + style.FramePadding.x * 2.0f +
			                                            style.ItemInnerSpacing.x) *
			                                               (components - 1)));
		...
		case ImGuiColorEditMode_HEX:
		{
			...
			value_changed |= ImGui::InputText("##Text", buf, IM_ARRAYSIZE(buf),
	...
	if (ImGui::IsItemHovered())
		ImGui::SetTooltip("Color:\n(%.2f,%.2f,%.2f,%.2f)\n#%02X%02X%02X%02X", col[0], col[1],
		                  col[2], col[3], IM_F32_TO_INT8(col[0]), IM_F32_TO_INT8(col[1]),
		                  IM_F32_TO_INT8(col[2]), IM_F32_TO_INT8(col[3]));
...
void ImGui::NextColumn()
{
	...
	if (window->DC.ColumnsCount > 1)
	{
		...
		{
			window->DC.ColumnsOffsetX = ImGui::GetColumnOffset(window->DC.ColumnsCurrent) -
			                            window->DC.ColumnsStartX + g.Style.ItemSpacing.x;
		...
		window->DC.CursorPos.x =
		    (float)(int)(window->Pos.x + window->DC.ColumnsStartX + window->DC.ColumnsOffsetX);
...
float ImGui::GetColumnOffset(int column_index)
{
	...
	const float content_region_width =
	    window->SizeContentsExplicit.x ? window->SizeContentsExplicit.x : window->Size.x;
	const float min_x = window->DC.ColumnsStartX;
	const float max_x = content_region_width - window->Scroll.x -
...
void ImGui::Columns(int columns_count, const char* id, bool border)
{
	...
	ImGui::PushID(
	    0x11223344); // Differentiate column ID with an arbitrary/random prefix for cases where
	                 // users name their columns set the same as another non-scope widget
		...
		for (int column_index = 0; column_index < columns_count + 1; column_index++)
		{
			...
			const float t = window->DC.StateStorage->GetFloat(
			    column_id, default_t); // Cheaply store our floating point value inside the integer
			                           // (could store an union into the map?)
			window->DC.ColumnsOffsetsT[column_index] = t;
		}
...
void ImGui::ShowMetricsWindow(bool* opened)
{
	if (ImGui::Begin("ImGui Metrics", opened))
	{
		...
		struct Funcs
		{
			...
			{
				bool node_opened = ImGui::TreeNode(
				     ...
				     pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
				{
					...
					{
						ImGui::BulletText(
						    "Draw %d indexed vtx, tex = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)",
						    pcmd->ElemCount, pcmd->TextureId, pcmd->ClipRect.x, pcmd->ClipRect.y,
						...
						if (show_clip_rects && ImGui::IsItemHovered())
						{
							...
							clip_rect.Round();
							GImGui->OverlayDrawList.AddRect(clip_rect.Min, clip_rect.Max,
							                                ImColor(255, 255, 0));
							vtxs_rect.Round();
							GImGui->OverlayDrawList.AddRect(vtxs_rect.Min, vtxs_rect.Max,
							                                ImColor(255, 0, 255));
		...
		g.DisableHideTextAfterDoubleHash++; // Not exposed (yet). Disable processing that hides text
		                                    // after '##' markers.
		...
		if (ImGui::TreeNode("Popups", "Opened Popups Stack (%d)", g.OpenedPopupStack.Size))
		{
			for (int i = 0; i < g.OpenedPopupStack.Size; i++)
			{
				...
				ImGui::BulletText(
				    "PopupID: %08x, Window: '%s'%s%s", g.OpenedPopupStack[i].PopupID,
