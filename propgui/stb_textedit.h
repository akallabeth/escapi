...
// [ImGui] we made a fix for using the END key on multi-line text edit, see
// https://github.com/ocornut/imgui/issues/275
...
//    STB_TexteditState *state, int key)
//
...
static void stb_textedit_discard_undo(StbUndoState* state)
{
	if (state->undo_point > 0)
	{
		...
		if (state->undo_rec[0].char_storage >= 0)
		{
			...
			memmove(state->undo_char, state->undo_char + n,
			        (size_t)((size_t)state->undo_char_point * sizeof(STB_TEXTEDIT_CHARTYPE)));
			for (i = 0; i < state->undo_point; ++i)
				if (state->undo_rec[i].char_storage >= 0)
					state->undo_rec[i].char_storage =
					    state->undo_rec[i].char_storage -
					    (short)n; // vsnet05 // @OPTIMIZE: get rid of char_storage and infer it
		}
		--state->undo_point;
		memmove(state->undo_rec, state->undo_rec + 1,
	...
	if (state->redo_point <= k)
	{
		...
		if (state->undo_rec[k].char_storage >= 0)
		{
			...
			memmove(state->undo_char + state->redo_char_point,
			        ...
			        (size_t)((size_t)(STB_TEXTEDIT_UNDOSTATECOUNT - state->redo_char_point) *
		...
		++state->redo_point;
		memmove(state->undo_rec + state->redo_point - 1, state->undo_rec + state->redo_point,
