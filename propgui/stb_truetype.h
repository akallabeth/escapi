...
extern "C"
{
	...
	STBTT_DEF void
	stbtt_GetBakedQuad(stbtt_bakedchar* chardata, int pw, int ph, // same data as above
...
#define STBTT_POINT_SIZE(x) (-(x))
	...
	STBTT_DEF void
	stbtt_GetPackedQuad(stbtt_packedchar* chardata, int pw, int ph, // same data as above
	                    int char_index,                             // character to display
	                    float* xpos,
	                    float* ypos,           // pointers to current position in screen pixel space
	                    stbtt_aligned_quad* q, // output: quad to draw
		...
		else
		{
			...
			if (x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len)
			{
				...
				else
				{
					...
					scanline[x2] +=
					    area + sign * (1 - ((x2 - x2) + (x_bottom - x2)) / 2) * (y1 - y_crossing);
