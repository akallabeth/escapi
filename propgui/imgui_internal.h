...
// Implement maths operators for ImVec2 (disabled by default to not collide with using
...
IMGUI_API void* ImLoadFileToMemory(const char* filename, const char* file_open_mode,
...
// Helpers: Math
// We are keeping those not leaking to the user by default, in the case the user has implicit cast
// operators between ImVec2 and its own types (when IM_VEC2_CLASS_EXTRA is defined)
...
{
	return ImVec4(lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z, lhs.w - lhs.w);
}
#endif
...
enum ImGuiButtonFlags_
{
	...
	ImGuiButtonFlags_PressedOnClick =
	    1 << 1, // return pressed on click only (default requires click+release)
	ImGuiButtonFlags_PressedOnRelease =
	    1 << 2, // return pressed on release only (default requires click+release)
...
// Simple column measurement currently used for MenuItem() only. This is very short-sighted for now
// and not a generic helper.
...
struct ImGuiState
{
	...
	float FontSize;     // (Shortcut) == FontBaseSize * g.CurrentWindow->FontWindowScale ==
	                    // window->FontSize()
	float FontBaseSize; // (Shortcut) == IO.FontGlobalScale * Font->Scale * Font->FontSize. Size of
	                    // characters.
	...
	ImGuiWindow* MovedWindow; // Track the child window we clicked on to move a window. Pointer is
	                          // only valid if ActiveID is the "#MOVE" identifier of a window.
	...
	float DragCurrentValue; // Currently dragged value, always float, not rounded by end-user
	                        // precision settings
	...
	ImGuiTextBuffer* LogClipboard; // Else log to clipboard. This is pointer so our GImGui static
...
// FIXME: That's theory, in practice the delimitation between ImGuiWindow and ImGuiDrawContext is
...
struct IMGUI_API ImGuiDrawContext
{
	...
	ImVec2 CursorMaxPos; // Implicitly calculate the size of our contents, always extending. Saved
	                     // into window->SizeContents at the end of the frame
	...
	float ColumnsStartX;  // Indentation / start position from left of window (increased by
...
struct IMGUI_API ImGuiWindow
{
	...
	ImVec2 WindowPadding; // Window padding at the time of begin. We need to lock it, in particular
	                      // manipulation of the ShowBorder would have an effect
	...
	int BeginCount; // Number of Begin() during the current frame (generally 0 or 1, 1+ if appending
	...
	int HiddenFrames;
	int SetWindowPosAllowFlags;  // bit ImGuiSetCond_*** specify if SetWindowPos() call will succeed
	                             // with this particular flag.
	int SetWindowSizeAllowFlags; // bit ImGuiSetCond_*** specify if SetWindowSize() call will
	                             // succeed with this particular flag.
	int SetWindowCollapsedAllowFlags; // bit ImGuiSetCond_*** specify if SetWindowCollapsed() call
	                                  // will succeed with this particular flag.
	bool SetWindowPosCenterWanted;

	ImGuiDrawContext DC; // Temporary per-window data, reset at the beginning of the frame
	ImVector<ImGuiID>
	    IDStack;     // ID stack. ID are hashes seeded with the value at the top of the stack
	ImRect ClipRect; // = DrawList->clip_rect_stack.back(). Scissoring / clipping rectangle. x1, y1,
	                 // x2, y2.
	...
	int FocusIdxAllRequestNext; // Item being requested for focus, for next update (relies on layout
	                            // to be stable between the frame pressing TAB and the next frame)
  ...
  public:
	...
	ImU32 Color(ImGuiCol idx, float a = 1.f) const
	{
		...
		c.w *= GImGui->Style.Alpha * a;
		return ImGui::ColorConvertFloat4ToU32(c);
	}
	ImU32 Color(const ImVec4& col) const
	{
		ImVec4 c = col;
...
namespace ImGui
{
	...
	IMGUI_API void RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border = true,
	                           float rounding = 0.0f);
	IMGUI_API void RenderCollapseTriangle(ImVec2 p_min, bool opened, float scale = 1.0f,
	                                      bool shadow = false);
	IMGUI_API void RenderCheckMark(ImVec2 pos, ImU32 col);

	IMGUI_API bool ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held,
	                              bool allow_key_modifiers, ImGuiButtonFlags flags = 0);
	...
	IMGUI_API bool SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min,
	                              float v_max, float power, int decimal_precision, bool horizontal);
	...
	IMGUI_API bool DragIntN(const char* label, int* v, int components, float v_speed, int v_min,
	...
	IMGUI_API bool TreeNodeBehaviorIsOpened(
	    ImGuiID id, ImGuiTreeNodeFlags flags = 0); // Consume previous SetNextTreeNodeOpened() data,
