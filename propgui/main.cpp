...
void devicewindow(int device)
{
	...
	if (isCaptureDone(device))
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 256, 256, 0, GL_RGBA, GL_UNSIGNED_BYTE,
		             (GLvoid*)capture[device].mTargetBuf);

		// ..and ask for more
		doCapture(device);
	}
	bool opened = true;
	ImGui::Begin(camname, &opened,
...
int main(int, char**)
{
	...
	SDL_Window* window =
	    SDL_CreateWindow("ESCAPI complex example", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
