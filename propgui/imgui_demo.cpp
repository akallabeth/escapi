...
void ImGui::ShowTestWindow(bool* opened)
{
	...
	if (ImGui::CollapsingHeader("Window options"))
	{
			...
			for (int i = 0; i < atlas->Fonts.Size; i++)
			{
				...
				if (ImGui::TreeNode("Details"))
				{
					...
					for (int config_i = 0; config_i < font->ConfigDataCount; config_i++)
						ImGui::BulletText("Input %d: \'%s\'", config_i,
						                  font->ConfigData[config_i].Name);
					ImGui::TreePop();
				}
				ImGui::TreePop();
			}
		...
		if (ImGui::TreeNode("Logging"))
		{
			ImGui::TextWrapped(
			    ...
			    "of a window or a block. Tree nodes can be automatically expanded. You can also "
			    "call ImGui::LogText() to output directly to the log without a visual output.");
			ImGui::LogButtons();
			ImGui::TreePop();
		}
	}
	...
	if (ImGui::CollapsingHeader("Widgets"))
	{
		...
		if (ImGui::TreeNode("Word Wrapping"))
		{
			...
			ImGui::GetWindowDrawList()->AddRectFilled(
			    ...
			    ImVec2(pos.x + wrap_width + 10, pos.y + ImGui::GetTextLineHeight()), 0xFFFF00FF);
			ImGui::PushTextWrapPos(ImGui::GetCursorPos().x + wrap_width);
			ImGui::Text("lazy dog. This paragraph is made to fit within %.0f pixels. The quick "
		...
		ImGui::Combo(
		    "combo", &item,
		    "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0"); // Combo using values packed in a single constant
		                                         // string (for really quick combo)
		...
		ImGui::Combo(
		    ...
		    IM_ARRAYSIZE(
		        items)); // Combo using proper array. You can also pass a callback to retrieve array
		                 // value, no need to create/copy an array just for that.
	...
	if (ImGui::CollapsingHeader("Graphs widgets"))
	{
		...
		ImGui::PlotLines("##Lines", values.Data, values.Size, values_offset, "avg 0.0", -1.0f, 1.0f,
		                 ImVec2(0, 80));
	...
	if (ImGui::CollapsingHeader("Layout"))
	{
		...
		{
			ImGui::TextWrapped(
			    "(Use ImGui::SameLine() to keep adding items to the right of the preceeding item)");
		...
		{
			ImGui::TextWrapped(
			    "(Using ImGui::BeginGroup()/EndGroup() to layout items. BeginGroup() basically "
			    "locks the horizontal position. EndGroup() bundles the whole group so that you can "
			    "use functions such as IsItemHovered() on it.)");
			ImGui::BeginGroup();
			{
				ImGui::BeginGroup();
				ImGui::Button("AAA");
		...
		{
			ImGui::TextWrapped(
			    "(This is testing the vertical alignment that occurs on text to keep it at the "
			    "same baseline as widgets. Lines only composed of text or \"small\" widgets fit in "
			    "less vertical spaces than lines with normal widgets)");
		...
		if (ImGui::TreeNode("Clipping"))
		{
			...
			ImGui::TextWrapped(
			    "On a per-widget basis we are occasionally clipping text CPU-side if it won't fit "
			    "in its frame. Otherwise we are doing coarser clipping + passing a scissor "
			    "rectangle to the renderer. The system is designed to try minimizing both "
			...
			ImGui::GetWindowDrawList()->AddText(
			    ImGui::GetWindowFont(), ImGui::GetWindowFontSize() * 2.0f,
	...
	if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus"))
	{
		...
		{
			ImGui::TextWrapped("You can use ImGui::GetItemActiveDragDelta() to query for the "
		...
		if (ImGui::TreeNode("Keyboard & Mouse State"))
		{
			...
			ImGui::Text("KeyMods: %s%s%s", io.KeyCtrl ? "CTRL " : "", io.KeyShift ? "SHIFT " : "",
			            io.KeyAlt ? "ALT " : "");
		...
		if (ImGui::TreeNode("Mouse cursors"))
		{
			ImGui::TextWrapped(
			    ...
			    "ImGui::GetMouseCursor() returns. You can also set io.MouseDrawCursor to ask ImGui "
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
	...
	{
		ImGui::SliderFloat(
		    "Alpha", &style.Alpha, 0.20f, 1.0f,
		    "%.2f"); // Not exposing zero here so user doesn't "lose" the UI. But application code
	...
	if (ImGui::TreeNode("Colors"))
	{
		...
		if (ImGui::Button("Output Colors"))
		{
			...
			for (int i = 0; i < ImGuiCol_COUNT; i++)
			{
				...
				if (!output_only_modified ||
				    memcmp(&col, (ref ? &ref->Colors[i] : &def.Colors[i]), sizeof(ImVec4)) != 0)
		...
		for (int i = 0; i < ImGuiCol_COUNT; i++)
		{
			...
			if (memcmp(&style.Colors[i], (ref ? &ref->Colors[i] : &def.Colors[i]),
			           sizeof(ImVec4)) != 0)
			{
				ImGui::SameLine();
...
static void ShowExampleAppFixedOverlay(bool* opened)
{
	...
	if (!ImGui::Begin("Example: Fixed Overlay", opened, ImVec2(0, 0), 0.3f,
...
static void ShowExampleAppManipulatingWindowTitle(bool* opened)
{
	...
	// You can use the "##" and "###" markers to manipulate the display/ID. Read FAQ at the top of
	// this file!
...
static void ShowExampleAppCustomRendering(bool* opened)
{
	...
	// Here we are using InvisibleButton() as a convenience to 1) advance the cursor and 2) allows
	// us to use IsItemHovered() However you can draw directly and poll mouse/keyboard by yourself.
	// You can manipulate the cursor using GetCursorPos() and SetCursorPos(). If you only use the
	// ImDrawList API, you can notify the owner window of its extends by using SetCursorPos(max).
	...
	draw_list->AddRectFilledMultiColor(
	    ...
	    ImColor(0, 0, 0), ImColor(255, 0, 0), ImColor(255, 255, 0), ImColor(0, 255, 0));
	draw_list->AddRect(canvas_pos,
	                   ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y),
	                   ImColor(255, 255, 255));

	draw_list->AddBezierCurve(
	    ImVec2(canvas_pos.x + 20, canvas_pos.y + 20), ImVec2(canvas_pos.x + 100, canvas_pos.y + 20),
	    ImVec2(canvas_pos.x + canvas_size.x - 100, canvas_pos.y + canvas_size.y - 20),
	    ImVec2(canvas_pos.x + canvas_size.x - 20, canvas_pos.y + canvas_size.y - 20),
	    ImColor(255, 200, 0), 5.0f);
	...
	draw_list->PushClipRect(ImVec4(
	    canvas_pos.x, canvas_pos.y, canvas_pos.x + canvas_size.x,
...
struct ExampleAppConsole
{
	...
	void Draw(const char* title, bool* opened)
	{
		...
		// You can seek and display only the lines that are visible - CalcListClipping() is a helper
		// to compute this information. If your items are of variable size you may want to implement
		// code similar to what CalcListClipping() does. Or split your data into fixed height items
		...
		for (int i = 0; i < Items.Size; i++)
		{
			...
			if (!filter.PassFilter(item))
				continue;
			ImVec4 col =
			    ImColor(255, 255, 255); // A better implementation may store a type per-item. For
			                            // the sample let's just parse the text.
